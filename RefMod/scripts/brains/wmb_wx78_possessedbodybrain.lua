...
local function ShouldMoveAnyways(inst)
    ...
    if leader ~= nil and target ~= nil then
		...
		if using_drone then
			...
			--return attack_range >= leader_dist --什么叫攻击距离大于领队距离就跑?
			--攻击距离略大于目标距离且目标没有位于不可到达的位置
			local x, y, z = target.Transform:GetWorldPosition()
			return (attack_range > dist_to_target + 4) and not (TheWorld.Map:IsOceanAtPoint(x, 0, z) or TheWorld.Map:IsGroundTargetBlocked(Vector3(x, 0, z)))
		end
		...
		if min_range and inst.sg:HasStateTag("spinning") then
			--if dist_to_target >= min_range - 2 and dist_to_target <= attack_range + 0.5 then
				--return false
			--end
			return min_range > dist_to_target + 2
...
local SEE_WORK_DIST = 12 --用于找领队附近的目标
local KEEP_WORKING_DIST = 16 --用于保持工作
local REPEAT_WORK_DIST = 4 --用于找自己身边的目标
...
do
	...
	local function IsAutoFightTarget(target, leader)
		--范围检测
		if not target:IsNear(leader, KEEP_WORKING_DIST) then
			return false
		end
	
		--不要内斗
		if target.prefab == "wx78_possessedbody" or HasFriendlyLeader(target) then
			return false
		end
		...
		if _target then
			--附身底盘
			if _target.prefab == "wx78_possessedbody" then
				return true
			end
	...
	function WMB_AutoModeFunc:FindHostileTarget(wx)
		local leader = CanAutoFight(wx); if not leader then return end
		if not wx.components.combat then return end
		
		--检查记录
		local target = wx.components.combat.target
		...
		--找自己身边的
		if target == nil then		
			target = FindEntity(wx, REPEAT_WORK_DIST, function(ent)
				return ent:IsNear(leader, SEE_WORK_DIST) and IsAutoFightTarget(ent, leader)
		...
		if target ~= nil then
			wx.components.combat:SetTarget(target)
			EquipBestWeapon(wx, target)
		end
	end
...
--是否为有效工作目标
local function IsAutoWorkTaget(target, leader, action)
	if target:HasOneOfTags(TOWORK_CANT_TAGS)
		or not target:IsNear(leader, KEEP_WORKING_DIST)
		or not target:IsValid()
		or not target.entity:IsVisible()
		or not target:IsOnValidGround()
		or not target.components.workable
		or not target.components.workable:CanBeWorked()
		or target.components.workable:GetWorkAction() ~= action
	then 
		return false
	end
...
--尝试获取工作
local function TryGetWork(wx, target, action)
	if target ~= nil and HasToolForAction(wx, action, true) then
		local tool = GetTool(wx)
		if tool and CanToolDoAction(tool, action) then		
			return BufferedAction(wx, target, action, tool)
		end
	end
end
...
do
	...
	local TOWORK_TAGS = { action.id.."_workable" }
	local TOWORK_ONE_OF_TAGS = { "stump", "farm_debris" }
	
	function WMB_AutoModeFunc:Dig(wx)
		local leader = CanAutoWork(wx); if not leader then return end

		--检查记录
		local target = wx.sg.statemem.target
		if target ~= nil and not IsAutoWorkTaget(target, leader, action) then
			target = nil
		end		
		...
		if target == nil then		
			...
			end, TOWORK_TAGS, TOWORK_CANT_TAGS, TOWORK_ONE_OF_TAGS)
		end
		
		--找领队附近的
		if target == nil then
			target = FindEntity(leader, SEE_WORK_DIST, nil, TOWORK_TAGS, TOWORK_CANT_TAGS, TOWORK_ONE_OF_TAGS)
		end

		return TryGetWork(wx, target, action)
	end
end
...
do
	...
	function WMB_AutoModeFunc:Chop(wx)
		local leader = CanAutoWork(wx); if not leader then return end

		--检查记录
		local target = wx.sg.statemem.target
		if target ~= nil and (not IsAutoWorkTaget(target, leader, action) or not CanWork(target)) then
			target = nil
		end		
		...
		if target == nil then		
			...
			end, TOWORK_TAGS, TOWORK_CANT_TAGS)
		end
...
do
	...
	local function CanWork(ent)
        ...
        if ent.components.growable ~= nil then
            return ent.components.growable:GetStage() >= 3
        end
		
		return true
	end
	
	function WMB_AutoModeFunc:Mine(wx)
		local leader = CanAutoWork(wx); if not leader then return end
...
--收获(只收获巨大作物)
do
	local TOWORK_TAGS = { "farm_plant" }

	--是否可以收获
	local function CanWork(ent)
		return ent.is_oversized and ent.components.pickable ~= nil and ent.components.pickable:CanBePicked()
	end

	function WMB_AutoModeFunc:Harvest(wx)
		local leader = CanAutoWork(wx); if not leader then return end
		...
		if target ~= nil then
			return BufferedAction(wx, target, ACTIONS.PICK)
		end
	end
end
...
--锤(只锤巨大作物)
do
	local action = ACTIONS.HAMMER
	local TOWORK_TAGS = { action.id.."_workable", "oversized_veggie", "show_spoilage" }
	...
	function WMB_AutoModeFunc:Hammer(wx)
		local leader = CanAutoWork(wx); if not leader then return end
...
--耕地
do
	local action = ACTIONS.TILL

	function WMB_AutoModeFunc:Till(wx)
		local leader = CanAutoWork(wx); if not leader then return end

		if HasToolForAction(wx, action, true) then
			local tool = GetTool(wx)
			if tool and CanToolDoAction(tool, action) then		
				--引用官方鱼人耕地逻辑
				local fns = BrainCommon.AssistLeaderDefaults["TILL"]
				if fns and fns.FindNew then
					return fns.FindNew(wx, KEEP_WORKING_DIST, SEE_WORK_DIST)
				end
			end
		end
	end
        ...
        PriorityNode({
           ...
           -- DoAction(self.inst, DoUpgradeModuleAction, nil, true),
			...
			WhileNode(function()
				...
				if leader ~= nil and (leader.WMB_REMOTE_AUTO_FIGHT or leader.WMB_REMOTE_AUTO_WORK) then
					return true
				end
				return false
                ...
                IfNode(
                    function() 
						...
						if leader ~= nil and leader.WMB_REMOTE_AUTO_FIGHT then
							--排除电刑机
							local weapon = GetTool(self.inst)
							if weapon and weapon:HasTag("wx_remotecontroller") then
								return false
							end
							return not ShouldMoveAnyways(self.inst) and (IsLeaderAttacking(self.inst) or not IsLeaderMoving(self.inst))
						end
					end, "WMB_AUTO_FIGHT",
					SequenceNode({
						ActionNode(function() WMB_AutoModeFunc:FindHostileTarget(self.inst) end),
						ChaseAndAttack(self.inst, MAX_CHASE_TIME, MAX_CHASE_DIST),
					})
                ),
                ...
                IfNode(
                    function() 
						local leader = GetLeader(self.inst)
						if leader ~= nil and leader.WMB_REMOTE_AUTO_WORK and self.inst.sg.statemem.target == nil then
							if not self.inst.components.combat:HasTarget() then							
								return true
							end
						end
					end, "WMB_AUTO_WORK",
                    SelectorNode({
                        IfNode(
                            function()
                                return HasToolForAction(self.inst, ACTIONS.DIG)
                            end, "WMB_AUTO_DIG",
                            DoAction(self.inst, function() return WMB_AutoModeFunc:Dig(self.inst) end, "WMB_AUTO_DIG_DOACTION", true)
                        ),
                        IfNode(
                            function()
                                return HasToolForAction(self.inst, ACTIONS.CHOP)
                            end, "WMB_AUTO_CHOP",
                            DoAction(self.inst, function() return WMB_AutoModeFunc:Chop(self.inst) end, "WMB_AUTO_CHOP_DOACTION", true)
                        ),
                        IfNode(
                            function()
                                return HasToolForAction(self.inst, ACTIONS.MINE)
                            end, "WMB_AUTO_MINE",
                            DoAction(self.inst, function() return WMB_AutoModeFunc:Mine(self.inst) end, "WMB_AUTO_MINE_DOACTION", true)
                        ),
                        IfNode(
                            function()
                                return true
                            end, "WMB_AUTO_HARVEST",
                            DoAction(self.inst, function() return WMB_AutoModeFunc:Harvest(self.inst) end, "WMB_AUTO_HARVEST_DOACTION", true)
                        ),
                        IfNode(
                            function()
                                return HasToolForAction(self.inst, ACTIONS.HAMMER)
                            end, "WMB_AUTO_HAMMER",
                            DoAction(self.inst, function() return WMB_AutoModeFunc:Hammer(self.inst) end, "WMB_AUTO_HAMMER_DOACTION", true)
                        ),
                        IfNode(
                            function()
                                return HasToolForAction(self.inst, ACTIONS.TILL)
                            end, "WMB_AUTO_TILL",
                            DoAction(self.inst, function() return WMB_AutoModeFunc:Till(self.inst) end, "WMB_AUTO_TILL_DOACTION", true)
                        ),
                    })
                ),
			...
			SequenceNode{
                ConditionWaitNode(function()
					--防止旋转时乱跑
					if self.inst.sg:HasAnyStateTag("prespin", "spinning") then
						return false
					end
